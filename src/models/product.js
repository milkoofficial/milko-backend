...
const createProduct = async (productData) => {
  const { name, description, pricePerLitre, imageUrl, isActive = true } = productData;

  const result = await query(
    `INSERT INTO products (name, description, price_per_litre, image_url, is_active, created_at, updated_at)
     VALUES ($1, $2, $3, $4, $5, NOW(), NOW())
     RETURNING *`,
    [name, description, pricePerLitre, imageUrl, isActive]
  );
